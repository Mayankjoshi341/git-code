...
data = pd.read_csv(r"C:\Users\mayan\OneDrive\git-projects\git-code\MyData-Warehouse\projects\MEDICSCAN\disease_data")
# Important: your OneHotEncoder has the list of symptoms it was trained on
all_symptoms = ['abdominal_pain','abnormal_menstruation','acidity','acute_liver_failure','altered_sensorium','anxiety','back_pain','belly_pain','blackheads','bladder_discomfort','blister',
 'blood_in_sputum','bloody_stool','blurred_and_distorted_vision', 'breathlessness', 'brittle_nails', 'bruising', 'burning_micturition', 'chest_pain', 'chills',
 ...
 'distention_of_abdomen','dizziness', 'drying_and_tingling_lips', 'enlarged_thyroid', 'excessive_hunger', 'extra_marital_contacts', 'family_history', 'fast_heart_rate', 'fatigue','fluid_overload',
 'foul_smell_ofurine',
...
if st.button("Predict"):
    if not selected_symptoms:
        ...
        st.subheader(f"🔍 Predicted Disease: **{decoder.inverse_transform([prediction])[0]}**")
        try:
            dis = data[data["Disease" == 'Drug Reaction']]
            st.write(dis)
        except:
            st.write("Nothing to show here")

        st.write("### 📊 Confidence")
        st.write(f"{np.max(probs) * 100:.2f}%")

        # Show top 3 possible diseases
