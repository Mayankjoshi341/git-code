...
if st.button("Predict"):
    if not selected_symptoms:
        ...
        try:
            ...
            st.write(data_row["Description"].values[0])
            st.subheader("Precautions to keep in mind")
            st.write(f"1) {data_row["Precaution_1"].values[0]}")
            st.write(f"2) {data_row["Precaution_2"].values[0]}")
            st.write(f"3) {data_row["Precaution_3"].values[0]}")
            st.write(f"4) {data_row["Precaution_4"].values[0]}")
            st.subheader("Recommended home remedies ")
            st.write(data_row["Home remedy"].values[0])
        except:
